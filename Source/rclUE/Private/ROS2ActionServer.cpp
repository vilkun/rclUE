...
void UROS2ActionServer::SendGoalResponse()
{
    ...
    // GoalResponse->accepted = true;//todo
    float TimeOfResponse = UGameplayStatics::GetTimeSeconds(reinterpret_cast<UObject*>(GetWorld()));
    // GoalResponse->stamp.sec = static_cast<int32>(TimeOfResponse); //todo
    uint64 ns = (uint64)(TimeOfResponse * 1e+09f);
    // GoalResponse->stamp.nanosec = static_cast<uint32>(ns - (GoalResponse->stamp.sec * 1e+09)); //todo
